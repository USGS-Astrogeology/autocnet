...
class NetworkCandidateGraph(CandidateGraph):
    ...
    def reapply(self, function, job_counter, chunksize=1000, arraychunk=None, walltime="01:00:00"):
        """
        Similar to apply function without the redis queue population. This function is intended for use
        on a redis queue generated by ncg.apply() that was not full emptied by slurm jobs due to errors
        on the slurm side of operations.

        Parameters
        ----------
        function:    string
                     The name of the function to apply (e.g.: "spatial.overlap.place_points_in_overlap")

        job_counter: int
                     The total number of jobs to resubmit.
        ...
        walltime:    string
                     In the format "Hour:Minute:Second" (e.g.: "00:30:00" is 30 minutes of walltime)
        """

        # Submit the left over jobs
        rconf = self.config['redis']
        rhost = rconf['host']
        rport = rconf['port']
        processing_queue = rconf['processing_queue']
        ...
        submitter = Slurm(command,
                          ...
                          output=self.config['cluster']['cluster_log_dir']+f'/autocnet.{function}-%j')
        submitter.submit(array='1-{}%{}'.format(job_counter, arraychunk), chunksize=chunksize)
        return '1-{}%{}'.format(job_counter, arraychunk)
